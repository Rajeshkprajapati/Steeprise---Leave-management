...
function SearchCompany(_this) {
    ...
    if (filters) {
        jFilters = JSON.parse(filters);
        if (_this.checked) {
            jFilters.CompanyUserId.push(_this.value);
        }
        else {
            let indexToRm = jFilters.CompanyUserId.indexOf(_this.value);
            if (indexToRm > -1) {
                jFilters.CompanyUserId.splice(indexToRm, 1);
            }
        }
    }
    getFilteredData(jFilters);
...
function getFilteredData(filters) {
    ...
    window.location.href = `/Job/SearchJobList/${queryString}`;
}
